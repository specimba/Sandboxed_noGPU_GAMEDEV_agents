...
/**
 * AFTERGLOW — pooled DOM damage numbers (view-side only).
 *
 * Consumes the sim's optional onFoeHurt notifications (src !== 'burn' is
 * filtered by the caller). Numbers rise 0.8 world-units over 0.6s
 * (easeOutCubic), fade over the final 0.25s, scale-pop 1 -> 1.15 in the
 * first 80ms. Chain hits render at 0.8x. Pool capped at 32; the oldest
 * entry is recycled when saturated. All timing is wall-clock and
...
export class DamageNumbers {
  ...
  update(camera: THREE.PerspectiveCamera, dt: number): void {
    ...
    for (const e of this.pool) {
      ...
      this.tmp.set(e.x, y, e.z).project(camera);
